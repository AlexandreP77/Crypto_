...
if [ -f "/app/update_data_by_hours.sh" ]; then
    cp /app/update_data_by_hours.sh /etc/periodic/hourly/
    chmod a+x /etc/periodic/hourly/update_data_by_hours.sh
    log "Scheduled script copied and permissions set"
else
    log "Error: /app/update_data_by_hours.sh not found!"
    exit 1
fi
